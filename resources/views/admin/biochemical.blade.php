...
<table class="table table-striped table table-bordered">
  ...
  @foreach($users as $user)
    <tr>
      <td>{{$user->id}}</td>
      <td>{{$user->username}}</td>
      @if($user->biochemicals->count()>0)
      <td>{{$user->biochemicals->count()}}</td>
      <td>
      {{ $user->biochemicals->last()->created_at}}
      </td>
      @else
        <td>0</td>
        <td>0</td>
        ...
        <div class="modal-body">
         @if($user->biochemicals->count() >0)
          ...
          <hr>
            ...
            @foreach ($user->biochemicals as $data)
             ...
             <div class="col-sm-3">
              ...
              <div class="col-sm-3">
              <a class="btn btn-danger" type="button" id="delete_biochemical" data-url ="{{route('delete.biochemical')}}" data-value="{{$data['id']}}"><i class="glyphicon glyphicon-remove" style="color: #fff"></i></a>
            </div>
               <hr>
           </div>
